...
public class Prob001_Vector {
	...
	private static Vector<Book> makeBookList() throws Exception {
		...
		File file = new File("C:\\study\\workspace\\javademo\\src\\java1015_collection\\prob\\booklist.txt");
		Scanner sc = new Scanner(file);
		Vector<Book> bookdata = new Vector<Book>();
		
		while(sc.hasNextLine()) {
			String[] str = sc.nextLine().split("/");
			Book b = new Book(str[0], str[1], str[2], str[3]);
			bookdata.add(b);
		}
